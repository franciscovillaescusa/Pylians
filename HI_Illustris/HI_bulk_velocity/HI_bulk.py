...
for num in snaps:
    ...
    halos = groupcat.loadHalos(snapshot_root, num, 
                               fields=['GroupLenType','GroupVel','GroupMass'])
    ...
    V_HI = np.zeros((len(halo_len),3), dtype=np.float64)
    ...
    for i in xrange(filenum):
        ...
        if not(done):
            done = HIL.V_HI_halos(pars, done, halo_len, halo_vel,
                                  MHI, vel, V_HI, M_HI)
                             
        Omega_HI += np.sum(MHI,  dtype=np.float64)

        print '\ni = %03d: z=%d'%(i,round(redshift))
    ...
    f.create_dataset('M',    data=halo_mass)
    f.create_dataset('V',    data=halo_vel)
    f.create_dataset('M_HI', data=M_HI)
    f.create_dataset('V_HI', data=V_HI)
    f.close()
