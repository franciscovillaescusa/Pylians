...
@cython.cdivision(True)
@cython.wraparound(False)
def void_finder(np.ndarray[np.float32_t, ndim=3] delta, float BoxSize, 
                float threshold, float Rmax, float Rmin, int threads):

    cdef float R = Rmax
    cdef float dist, R_gridx
    cdef long Nvoids = 0
    ...
    if Rmin<BoxSize*1.0/dims:
        print 'The minimum scale resolved in the density field is %.3f Mpc/h'\
            %(BoxSize*1.0/dims)
        print 'While the minimum scale is %.3f Mpc/h!!!'%(Rmin);  sys.exit()

    # find the maximum possible number of voids
    max_num_voids = int(BoxSize**3/(4.0*np.pi/3.0*Rmin**3))
    print 'maximum number of voids = %d'%max_num_voids
    ...
    for R in Radii:
        ...
        print 'void finding took %.3f seconds'%(time.time()-start)        
